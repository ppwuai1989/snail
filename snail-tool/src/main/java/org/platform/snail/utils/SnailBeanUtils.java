...
public class SnailBeanUtils extends BeanUtils {
	static {
	 ...
	 *            bean对象是否包含父类属性
	 */
	public static void copyBean2Map(Object bean, Map<String, Object> map, boolean containPrarent) {
		if (containPrarent) {
	 ...
	 * 
	 */
	public static void copyMap2Bean(Object bean, Map<String, ?> map) {
		if (bean == null || map == null)
			return;
		try {
		...
		for (PropertyDescriptor descriptor : propertyDescriptors) {
			...
			try {
				...
				if (String.class.isInstance(value)) {
					...
					SnailBeanUtils.setProperty(bean, name, newValue);
				}
			} catch (IllegalArgumentException e) {
				e.printStackTrace();
			} catch (IllegalAccessException e) {
