...
Ext.extend(keel.UploadPanel,Ext.Panel,{
	...
	onUploadSuccess : function(file, serverData) {
		...
		if (result.success) {			
			...
			uploadFileKeys.push(result.key);		
		}else{
			for(var i=0;i<ds.getCount();i++){
				var rec =ds.getAt(i);
				if(rec.get('id')==file.id){
...
//临时存放上传文件标识
var uploadFileKeys = new Array();
var uploadWin;
function uploadFile(obj){
		var saveFilePath = 'C:\\Users\\xiao\\Desktop\\uploadFiles\\';
		uploadFileKeys=new Array();
		uploadWin=new Ext.Window({
			...
			items : [
				{
					...
					uploadUrl : contextPath+"/files/upload.do",
					flashUrl : contextPath+'/content/js/common/uploader/swfupload.swf',
					filePostName : 'file', //后台接收参数
					fileTypes : '*.*',//可上传文件类型(多种类型：'*.jpg;*.gif' )
					postParams : {savePath:saveFilePath} //上传文件存放目录
				}
			],
			buttonAlign : 'center',
			buttons : [ {
			...
			handler : function() {
				uploadWin.hide();
				obj.setValue(uploadFileKeys.join(","));
			}
		}]
		}).show();
}